...
<font face="Source Code Pro " size = 3>由于Qt内建的一套隐式内存共享策略，即新对象引用共享数据只单纯让引用数+1，而不需要拷贝数据（这样使用值传递和const &传递效果基本一样了），只有在写时才会发生深拷贝操作。参考：[Implicit Sharing](https://doc.qt.io/qt-5/implicit-sharing.html)。
